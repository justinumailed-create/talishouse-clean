...
import { TALISBOOKS_PRODUCT_NAME, TALISBOOKS_SIDEBAR_ITEMS } from "@/lib/talisbooks/constants";
import { TALISBOOKS_ROUTES } from "@/lib/talisbooks/routes";

function isActive(pathname: string, href: string) {
  if (href === TALISBOOKS_ROUTES.DASHBOARD) {
    return pathname === href;
  }
  return pathname === href || pathname.startsWith(`${href}/`);
}

export default function TalisBooksSidebar() {
  ...
  return (
    <aside className="flex h-full w-60 shrink-0 flex-col border-r border-neutral-200/80 bg-white/80 backdrop-blur-xl">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
        {TALISBOOKS_SIDEBAR_ITEMS.map((item) => {
      ...
      <div className="border-t border-neutral-200/80 px-5 py-4 space-y-2">
        <Link
          href={TALISBOOKS_ROUTES.VIEWER}
          className="block rounded-xl bg-neutral-900 px-3 py-2.5 text-sm font-medium text-white transition-colors hover:bg-neutral-800"
        >
          Open Viewer
        </Link>
        <Link
