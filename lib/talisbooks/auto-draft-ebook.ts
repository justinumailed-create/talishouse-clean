 ...
 * Upload order: #1 front cover, #2 back cover, #3–20 interiors
 * (landscapes become two-page spreads; portraits stay single pages).
 */
...
export async function autoGenerateDraftTalisBook(
  ...
  if (exactPdf) {
    ...
    const pageRows = pageImageUrls.map((url, index) => {
  ...
  const [{ landscapes, coverImageUrl, backCoverImageUrl, galleryUrls }, agent] =
    await Promise.all([
    imageAssetsPromise,
