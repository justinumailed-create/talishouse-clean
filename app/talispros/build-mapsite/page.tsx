...
function SectionCard({
  ...
  return (
    <div className="border border-neutral-200 rounded-2xl bg-white overflow-hidden transition-shadow duration-200 hover:shadow-sm">
      ...
      <div id={id} role="region" className={`transition-all duration-200 ease-in-out ${isOpen ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
...
function ProvinceSelect({ value, onChange, country, error }: { value: string; onChange: (v: string) => void; country: string; error?: string }) {
  ...
  return (
    <div>
      ...
      <select
        ...
        className={`w-full h-11 px-4 bg-white border text-sm text-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900/20 transition-all rounded-xl appearance-none ${error ? "border-red-300" : "border-neutral-200"} ${!value ? "text-neutral-400" : ""}`}
...
export default function BuildMapsitePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      for (let i = 0; i < files.tebPictures.length; i++) fd.append(`tebPicture_${i}`, files.tebPictures[i]);
      fd.append("helpPreference", form.helpPreference);
      ...
      setSubmitError(`Connection error: ${err instanceof Error ? err.message : "Unknown"}`);
      ...
      <div className="min-h-screen bg-white flex items-center justify-center px-5 py-8">
        <div className="w-full max-w-lg text-center">
          ...
          <div className="flex flex-col gap-3">
            <Link href={`/talispros/register?fastCode=${generatedFastCode}`}
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
                <SectionCard number={1} title="General Information" description="Account identification and type." isOpen={openSections.has(1)} onToggle={() => toggleSection(1)}>
                  <div className="space-y-4">
                    ...
                    <InputField label="FAST Code" required value={form.fastCode} onChange={(v) => updateField("fastCode", v)} placeholder="e.g. JOHN-TORONTO" error={errors.fastCode} />
                    <p className="text-xs text-neutral-400 -mt-2">Identifies an Account</p>
                  </div>
                </SectionCard>
                ...
                <SectionCard number={3} title="Home PIN Location" description="Property Identification Number and location." isOpen={openSections.has(3)} onToggle={() => toggleSection(3)}>
                  <div className="space-y-4">
                    <InputField label="Property PIN" required value={form.homePin} onChange={(v) => updateField("homePin", v)} placeholder="e.g. 12345-6789" error={errors.homePin} />
                    <InputField label="Street Address" required value={form.homeAddress} onChange={(v) => updateField("homeAddress", v)} placeholder="123 Main St" autoComplete="street-address" error={errors.homeAddress} />
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <InputField label="City" required value={form.homeCity} onChange={(v) => updateField("homeCity", v)} placeholder="Toronto" autoComplete="address-level2" error={errors.homeCity} />
                      <ProvinceSelect value={form.homeProvince} onChange={(v) => updateField("homeProvince", v)} country={form.homeCountry} error={errors.homeProvince} />
                      <InputField label="Postal Code" required value={form.homePostalCode} onChange={(v) => updateField("homePostalCode", v)} placeholder="A1A 1A1" autoComplete="postal-code" error={errors.homePostalCode} />
                    </div>
                    <CountrySelect value={form.homeCountry} onChange={(v) => updateField("homeCountry", v)} />
                  </div>
                </SectionCard>

              <SectionCard number={4} title="Media Focus" description="Talis TV and Talis E-Books." isOpen={openSections.has(4)} onToggle={() => toggleSection(4)}>
              ...
              <SectionCard number={5} title="Help Us Improve" description="Future media functionality preferences." isOpen={openSections.has(5)} onToggle={() => toggleSection(5)}>
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      return (
                        <button key={opt.value} type="button" onClick={() => updateField("helpPreference", opt.value)}
              ...
              <SectionCard number={6} title="Terms" description="Consent and data processing." isOpen={openSections.has(6)} onToggle={() => toggleSection(6)}>
                <div>
